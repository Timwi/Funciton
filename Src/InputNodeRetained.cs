...
namespace Funciton
{
    sealed class InputNodeRetained(FuncitonFunction thisFunction, int inputPosition) : InputNode(thisFunction, inputPosition)
    {
        ...
        public override Node CloneForFunctionCall(int clonedId, Node[] functionInputs)
        {
            ...
            {
                _clonedId = clonedId;
                _cloned = new InputNodeRetained(_thisFunction, InputPosition) { _functionInputs = functionInputs };
            }
            return _cloned;
        }
